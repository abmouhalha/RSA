...
int main()
{
    ...
    mpz_set_str(M, "42", 10);  // Exemple de message : 42
    std::cout << "Original Message M = 42" << std::endl;
    
    // Encrypt the message
    mpz_powm(c, M, e, n);  // c = M^e mod n
    char c_str[1000];
    mpz_get_str(c_str, 10, c);
    std::cout << "Encrypted Message c = " << c_str << std::endl;
    ...
    std::cout << "Decrypted Message M = " << M_str << std::endl;
